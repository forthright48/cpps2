...
(function() {
  ...
  try {
    const child = spawn('sh', ['server/node_modules/queue/scripts/script.sh']);
    child.on('close', (code) => {
    ...
    child.on('error', (err) => {
      console.log('script emitted error', err);
    });
    child.stderr.on('data', (data) => {
      console.log('script emitted stderr', data.toString());
    });
  } catch (err) {
...
const {ojnamesOnly} = require(path.join(rootPath, './models/ojInfo'));

// Mongoose
require(path.join(rootPath, './config/database.js'));
const Gate = require(path.join(rootPath, './models/gateModel'));
